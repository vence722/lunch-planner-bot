package constant

const (
	TG_BOT_API_KEY = "6369942858:AAHw6cnDhOKXFLxpPsotVsPC0mYfY-Pos-U"

	DEFAULT_DATE_FORMAT = "2006年01月02日"

	MESSAGE_USAGE = `係咪唔知食乜？我可以幫到你😊！
唔該用'/add'添加餐廳名稱，每次可添加一個（重複名稱會合併）
用'/showall'查看當前已添加的所有餐廳名稱
然後用'/plan'就可以幫你plan好呢個禮拜每日食乜喇！😋`

	MESSAGE_RESTAURANT_ADDED = "餐廳加咗落list喇："

	MESSAGE_SHOW_ALL_RESTAURANTS = "已添加嘅餐廳有："

	MESSAGE_PLAN_ERROR = `出現咗個小問題🤔，係咪你未加夠五間餐廳？
你可以用'/showall'先確認下`

	MESSAGE_LUNCH_PLAN_WEEKLY = `我幫你plan好喇😊
呢個禮拜係 %s 至 %s
星期一： %s
星期二： %s
星期三： %s
星期四： %s
星期五： %s

希望你鍾意呢個plan😊！（唔滿意可以再'/plan'過🤔）`
)
